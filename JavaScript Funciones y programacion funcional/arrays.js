...
/* for (let it = 0; it < coches.length; it++) {
     cochesFiltrats.push(coches.at(it).filter(coche => (coche != "Kia" && coche != "Alfa Romeo" && 2026 - 2 >= 20)), [0, , 2])
 };*/



let newCoches = coches.filter(([marca, , ano] = coche) => marca !== "Kia" && marca !== "Alfa Romeo" && 2026 - ano >= 20);

//newCoches.sort();

        //newCoches.sort(([marcaA, , , preuA] = cocheA, [marcaB, , , preuB] = cocheB) =>parseInt(marcaB.charAt(0)) - parseInt(marcaA.charAt(0)));
        newCoches.sort();
        newCoches.reverse();

console.log(newCoches);
