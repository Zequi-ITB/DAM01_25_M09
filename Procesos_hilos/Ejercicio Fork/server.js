...
app.get('/entreno/:nombre', (req, res) => {
    console.log('Cliente pide imc. Delegando tarea...');
    ...
    cocineroHijo.send({ comando: "calcular imc", nom : nombre , pes : peso, alt:  altura});
    ...
    cocineroHijo.on('error', (error) => {
        ...
        res.status(500).json({ error: 'Fallo al calcular el imc' });
    });
});
