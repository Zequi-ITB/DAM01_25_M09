// Escuchamos el walkie-talkie (los mensajes del server.js)
process.on('message', (mensaje) => {

    // Verificamos qué orden nos ha dado el Jefe
    if (mensaje.comando === 'calcular imc') {
        console.log('[Hijo] Recibida orden de calcular imc. Empezando trabajo pesado...');
        console.log(mensaje.nom);
        // Simulamos un trabajo de CPU muy pesado (contar millones de cosas)
        let total = 0;
        for (let i = 0; i < 5_000_000_000; i++) {
            total++;
        }

        const resultado = (mensaje.pes / (Math.pow(mensaje.alt, 2))).toFixed(2);

        let pla = "";
        if (resultado < 20) {
            pla = "Volumen";
        }
        if (resultado > 20 && resultado < 30) {
            pla = "Definicion";
        }
        if (resultado > 30) {
            pla = "Mantenimiento";
        }

        console.log('[Hijo] Trabajo pesado completado.');
        // El trabajo terminó, avisamos al Jefe por el walkie-talkie
        process.send({
            estado: 'imc calculado con éxito',
            nombre : mensaje.nom,
            imc: resultado,
            plan: pla,
            calorias: 600
        });

        // Cerramos este proceso para no dejar cocineros "zombis" consumiendo RAM
        process.exit();
    }
});